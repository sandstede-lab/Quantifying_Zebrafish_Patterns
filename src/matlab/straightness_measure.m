function [top_cv, bottom_cv] = straightness_measure(cells_xanC, x_querys, xanS_betti_1, mel_betti_1, plotName, plotOn)
...
%
% Inputs: cells_xanC are the coordinates of X^C cells in the pattern of
% interest; x_querys are the x-coordinates of the grid points used for the
% numerical computation of the arc length distance of the stripe boundary; xanS_betti_1
% is the first betti number of X^S cells; mel_betti_1 is the first betti
% number of the M cells; plotName is save name of plot; and olotOn is a flag for plotting patterns (= 1 to plot, =0 to not plot)
%
...
% Apply single linkage clusters to X^c cells with number of clusters n,
% set to number of expected stripes (3) minus the number of stripe breaks,
% as indicated by the betti numbers.
Z_xanC = linkage(cells_xanC, 'single', '@my_dist_WT');

if xanS_betti_1 >= 2 || mel_betti_1 >= 2
    n = 3;
elseif xanS_betti_1 == 1 || mel_betti_1 == 1
    n = 2;
else
    n = 1;
end
c_xanC = cluster(Z_xanC,'maxclust', n);
...
for j = 1:n
    ...
    my_cluster = cells_xanC(find(c_xanC == j),:);
